...
async function addComponentCss(component,index,dependenciesFlat){
  ...
  try {
    var toRemove = currentDoc.
      querySelectorAll(`[${app.componentAttribute}="${component.name}"]`);
  }
  catch(e){
    console.error(e);
  ...
  var oldBundleLink;
  if(oldBundleLinkDep) {
    oldBundleLink = currentDoc.
      querySelector(`link[${app.componentAttribute}="${oldBundleLinkDep.name}"]`);
    oldBundleLink && app.core.htmlMatch.after(oldBundleLink, linkEl);
  }
  if(!oldBundleLink){
    for (var obli = index + 1; obli < dependenciesFlat.length; obli++) {
      oldBundleLink = currentDoc.
        querySelector(`link[${app.componentAttribute}="${dependenciesFlat[obli].name}"]`);
      if (oldBundleLink)
        break;
    }
    if(oldBundleLink)
...
function findBundleScripts(name){
    var possibleScripts = app.core.currentDocument.
        querySelectorAll(`script[${app.componentAttribute}="${name}"]`);
...
function selectButtonsContentEditable(elContentEditable){
  ...
  buttons.forEach(v=>{
      var tagName = v.textContent;
      v.disabled = !tagName.match(app.utils.phrasingTags);
  })
}
function selectButtonsToInsert(selectorObject){
  ...
  buttons.forEach(button=>{
      ...
      var tagName = button.textContent;
