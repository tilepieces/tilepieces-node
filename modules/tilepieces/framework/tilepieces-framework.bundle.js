...
TilepiecesCore.prototype.setHistory = async function (historyObject) {
  ...
  if (isStyleSheetModification) {
    ...
    history.stylesheets.push(newRecord);
  } else {
...
TilepiecesCore.prototype.undo = async function () {
  var $self = this;
  var history = $self.history;
  if (!history.entries.length || history.pointer == 0)
    return;
...
function assignFramesToMenuBar(toDock) {
  tilepieces.panels = frameUIEls.map(v => {
    ...
    li.onclick = async () => {
      ...
      d.show(true);
      closeMenuBar();
      await tilepieces.storageInterface.setSettings({ // no await
        "projectSettings": {
...
async function searchForLastProject() {
  ...
  var lastProject = projectSearchParam &&  Array.isArray(tilepieces.projects) &&
  tilepieces.projects.find(v=>v.name == projectSearchParam)? {name: projectSearchParam} : Array.isArray(tilepieces.projects) && tilepieces.projects[0];
  if (lastProject) {
...
function setProject(e){
  ...
  storageInterface.read(startPath)
    .then(res => {
      ...
      var sentence = "error in reading file "+startPath+" in current project '" + prName+"'";
...
refreshTrigger.addEventListener("click", e => {
  e.preventDefault();
  tilepieces.core.currentDocument.defaultView.location.reload(true);
});
