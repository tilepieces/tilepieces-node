...
tabVertical.addEventListener("click",e=>{
  tabVertical.classList.toggle("tab-vertical-open");
});
...
const projectsDialogTabs = document.getElementById("projects-dialog-tabs");
...
const componentsDialogTabs = document.getElementById("components-dialog-tabs");
let childrenOfComponents = [];
let projectsInnerTemplates = [];
const projectChildrenTemplate = document.getElementById("project-child");
const projectComponentUI= document.getElementById("project-component-UI");
const header_tab = document.getElementById("main-tab");
...
componentsDialog.addEventListener("click",e=>{
  ...
  opener.addEventListener("prompt-dialog-submit",async e=>{
      var name = (parent ? parent + "/" : "") + e.detail.value;
      try {
      ...
      if(parent){
        var hasClassList = target.parentNode.querySelector(".components-children");
        var hasDetail = target.parentNode.querySelector(".as-detail");
        if(hasClassList && hasClassList.classList.contains("open")){
            hasClassList.__project = app.localComponentsFlat[parent];
            hasClassList.click();
            hasClassList.click();
        }
        else if(hasDetail){
            var wasOpened = hasDetail.classList.contains("open");
            var firstLi = target.closest("li");
            var componentChildrenLi =
                firstLi.parentNode.classList.contains("list-components-root") ?
                    firstLi :
                    firstLi.parentNode.closest("li");
            var componentChildrenParent = componentChildrenLi.querySelector(".components-children");
            componentChildrenParent.__project = app.localComponentsFlat[componentChildrenParent.__project.name];
            if(hasClassList)
                return;
            componentChildrenParent.click();
            componentChildrenParent.click();
            var selectors = componentChildrenLi.
                querySelectorAll(".as-detail[data-parent='" + target.dataset.parent + "']");
            selectors.forEach((s,i,a)=>{
                if(i>=a.length-2 && !wasOpened)
                    return;
                s.click()
            });
        }
...
componentsDialog.addEventListener("click",async e=> {
  ...
  try {
      ...
      console.error(e);
      openerDialog.open(e.error || e.toString())
  }
});
...
globalComponents.addEventListener("change",e=>{
  changeComponentsCheck(e);
  globalComponentsUITemplate.set("",localComponentsUIMOdel);
});
// local
...
document.getElementById("export-global-components")
...
document.getElementById("export-local-components")
.addEventListener("click",async e=>{
    openerDialog.open("exporting components...", true);
    var componentsToExport = [];
...
async function importBundleInComponent(pkg,array = [],name = ""){
    if(array.find(a=>a.name == pkg.name))
        return array;
    /*
    var componentPath = ("/" + app.componentPath + "/" + (name ? name + "/" + pkg.path : pkg.name)).replace(/\/\//g,"/");
    var componentPath = app.componentPath
    app.storageInterface.delete(componentPath);
    */
    array.push(pkg);
    ...
    var miscellaneous = pkg.miscellaneous && pkg.miscellaneous.length ?
        await app.storageInterface.search("",
            name ?
            pkg.miscellaneous.map(v=>pkg.path+"/"+v) :
            pkg.miscellaneous,
            null,name || pkg.name) :
        {searchResult:[]};
    miscellaneous.searchResult.forEach(m=>{
        updateResourceFromComponent(m,name||pkg.name,null,"",mergeMiscellaneous);
    });
    var interfaceFiles = pkg.interfaceFiles && pkg.interfaceFiles.length ?
        await app.storageInterface.search("",
            name ?
            pkg.interfaceFiles.map(v=>pkg.path+"/"+v) :
            pkg.interfaceFiles
            ,null,name ||pkg.name) :
        {searchResult:[]};
    interfaceFiles.searchResult.forEach(m=>{
        updateResourceFromComponent(m,name||pkg.name);
    });
    // file to import
    var toImport = {
    ...
    for (var k in toImport) {
        ...
        if(k=="dependencies"){
            /*
            if(!pkg.addDependenciesToBundles)
                continue;*/
            ...
            for(var cName in v) {
                ...
                if(!pkgInner)
                    continue;
                array = await importBundleInComponent(pkgInner,array,name || pkg.name);
                array.push(pkgInner);
            }
        }
        else {
            v && await updateResourceFromComponent(
                v, name || pkg.name, pkg.name,pkg.path ? pkg.path+"/" : "");
        }
    }
...
async function updateResourceFromComponent(toImport,
    ...
    if (Array.isArray(toImport)) {
        ...
        await app.storageInterface.update(pathToUpdate,new Blob([fileText]));
    }
}
...
document.getElementById("import-local-components").addEventListener("change",async e=>{
    importingComponents(e,true)
});
document.getElementById("import-global-components").addEventListener("change",async e=>{
    importingComponents(e)
});
componentsDialog.addEventListener("click",async e=>{
    var t = e.target;
    if(!t.classList.contains("components-children"))
        return;
    ...
    for(var childName in childrens){
        ...
        var li = projectChildrenTemplate.content.cloneNode(true);
        new opener.TT(li,model,{
            templates : [{
                name : "project-component-UI",
                el : projectComponentUI.content
            }]
        });
...
componentsDialog.addEventListener("click",async e=>{
    ...
    var superList = target.closest("ul");
    var isLocal = target.closest("#local-components");
    try {
        var resultSettings = await app.storageInterface.
            deleteComponent({
                local: !!isLocal,
                component: {
                    name:project.name
        ...
        if (!resultSettings.result)
            throw resultSettings;
    }
    catch(e){
    ...
    if(!superList.classList.contains("list-components-root")){
        var hasClassList = target.closest("li").parentNode.closest("li").querySelector(".components-children");
        hasClassList.__project = app.localComponentsFlat[hasClassList.__project.name];
        hasClassList.click();
        hasClassList.click();
    }
})
componentsDialog.addEventListener("click",async e=>{
    ...
    d.events.on("close",async e=>{
        await app.getSettings();
        localComponentsUITemplate.set("localComponents",turnComponentsToArray(app.localComponents));
        componentSettingsSibling.parentNode.
        insertBefore(componentSettings,componentSettingsSibling);
        settingsFormActivation(app.isComponent);
        if(hidden)
...
componentsDialog.addEventListener("click",async e=> {
    ...
    try{
        openerDialog.open("fetching resource...", true, true);
        ...
        for(var i = 0;i<htmlfiles.searchResult.length;i++){
            ...
            openerDialog.open("fetching file path " + htmlfilePath + "...", true, true);
            ...
            if(toChange.length) {
                ...
                var s = tilepieces.createDocumentText(doc);
        ...
        console.error(e);
        openerDialog.open("error in set fixed html");
        return;
    }
    openerDialog.open("update done");
});
...
function addComponentProperty(obj){
    switch(obj){
        ...
        case "bundle.stylesheet":
            ...
            break;
        case "bundle.script":
            settingsModel.bundle.script.push({name : "attr-"+settingsModel.bundle.script.length, value : "",
...
async function submitSettings(){
    ...
    try {
        var setComponentInProject = await app.storageInterface.
            createComponent({
...
newProjectButton.addEventListener("click",e=>{
    ...
    opener.addEventListener("prompt-dialog-submit",createProject,{once:true});
});
...
function deleteProject(projectName) {
    opener.confirmDialog("Are you sure you want to delete " + projectName);
    opener.addEventListener("confirm-dialog-submit", ()=>{
    ...
    try{
      ...
      for(var i = 0;i<projects.length;i++){
          ...
          for(var f = 0;f<files.length;f++) {
              ...
              await app.storageInterface.
                  update(file.relativePath, new Blob([await file.file.async("arraybuffer")]));
          }
          delete p.path;
